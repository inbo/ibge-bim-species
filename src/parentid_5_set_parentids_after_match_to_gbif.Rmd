...
We check the completeness of the taxonomic tree as provided by GBIF. We search parents in not used taxa as well and we add them to the used taxa. 

```{r add_parents_in_unused_taxa}
used_taxa_gbif_parent_key <- 
  taxa_match_gbif %>%
  filter(used == FALSE) %>%
  filter(gbif_usageKey %in% used_taxa_gbif_parent_key$gbif_parentKey) %>%
  bind_rows(used_taxa_gbif_parent_key) %>%
  arrange(id)
```

Parents found in unused taxa:

```{r n_parents_in_used_unused_taxa}
used_taxa_gbif_parent_key %>%
  filter(used == FALSE) %>%
  nrow()
```

Number of taxa whose GBIF parent keys are (not) present in taxa:

```{r parent_in_db}
used_taxa_gbif_parent_key %>% 
  filter(!is.na(gbif_usageKey)) %>%
  group_by(gbif_parentKey %in% (used_taxa_gbif_parent_key$gbif_usageKey)) %>%
  summarize(n_taxa = n()) %>%
  rename(parent_in_db = "gbif_parentKey %in% (used_taxa_gbif_parent_key$gbif_usageKey)")
```

More detailed summary about used taxa with no parents:
...
used_taxa_gbif_parent_key %>%
  filter(!gbif_parentKey %in% used_taxa_gbif_parent_key$gbif_usageKey) %>%
...
used_taxa_gbif_parent_key_complete <-
  used_taxa_gbif_parent_key %>%
...
find_parentid <- function(parent_key) {
  ...
  if (nrow(parent_id) > 1) {
    # remove duplicates based on number of occurences
...
We can now add the new parent IDs to data.frame. In order to maintain changes and compare the new parent IDs with the old ones, we add these values in new column called `parentid_corrected`:
...
Following GBIF Backbone, the old `parentid` 26780 ([Rana	Linnaeus, 1758](https://www.gbif.org/species/2422253)), should be replaced by `parentid_corrected` 41810 ([]()):
