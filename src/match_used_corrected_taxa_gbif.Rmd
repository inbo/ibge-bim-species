---
title: "Match used taxa to GBIF Backbone after review"
author:
  - Damiano Oldoni
  - Lien Reyserhove
  - Peter Desmet
...
```{r match_used_corrected_taxa-match_used_corrected_taxa_to_gbif-load_libs}
...
## Read used taxa after review

We read the used taxa with the corrected name, rank and kingdom information:

```{r match_used_corrected_taxa-match_used_corrected_taxa_to_gbif-read_used_taxa}
used_taxa_after_correction <- read_tsv(
  here("data", "interim", "corrected_used_taxa.tsv"),
  col_types = cols(
...
The column `corrected` acts as a flag to indicate whether experts corrected any of  `acceptedname`, `scientificnameauthorship`, `bim_kingdom` or `taxonranken`. Number of used taxa after correction:

```{r match_used_corrected_taxa-match_used_corrected_taxa_to_gbif-n_corrected_in_use}
used_taxa_after_correction %>%
  group_by(corrected) %>%
  summarize(n_taxa = n())
```
...
```{r match_used_corrected_taxa-match_used_corrected_taxa_to_gbif-read_taxa_table_with_GBIF}
...
```{r match_used_corrected_taxa-match_used_corrected_taxa_to_gbif-type_of_match}
taxa_gbif %>%
...
First, we add the columns coming from GBIF to `used_taxa_after_correction`:

```{r match_used_corrected_taxa-match_used_corrected_taxa_to_gbif-add_GBIF_info_to_used_taxa_after_correction}
used_taxa_after_correction <-
  used_taxa_after_correction %>%
  left_join(taxa_gbif %>%
...
```{r match_used_corrected_taxa-match_used_corrected_taxa_to_gbif-n_taxa_corrected_is_true}
used_corrected_taxa <-
  used_taxa_after_correction %>%
...
```{r match_used_corrected_taxa-match_used_corrected_taxa_to_gbif-prepare-name}
used_corrected_taxa <-
  used_corrected_taxa %>%
...
```{r match_used_corrected_taxa-used_corrected_taxa_rank_for_gbif}
used_corrected_taxa <-
  used_corrected_taxa %>%
...
```{r match_used_corrected_taxa-gbif_terms}
...
```{r match_used_corrected_taxa-get_name_gbif, cache=TRUE}
used_corrected_taxa_gbif <-
  used_corrected_taxa %>%
...
```{r match_used_corrected_taxa-add_gbif_prefix}
cols_from_gbif <-
colnames(used_corrected_taxa_gbif)[
  colnames(used_corrected_taxa_gbif) %in% gbif_terms]
colnames(used_corrected_taxa_gbif)[
  colnames(used_corrected_taxa_gbif) %in% gbif_terms] <-
...
```{r match_used_corrected_taxa-overview_match}
used_corrected_taxa_gbif %>%
  group_by(gbif_matchType) %>%
  count()
```
...
```{r match_used_corrected_taxa-used_taxa_after_correction}
used_corrected_taxa_gbif %>%
  filter(gbif_matchType != "EXACT")
```

Notice how the rank of most of the unmached taxa is one of: subclass, division, subphylum, subclass, subfamily, superfamily. Such ranks are not valid in GBIF Backbone.

## Add retrieved GBIF information to used taxa
...
```{r match_used_corrected_taxa-merge_corrected_taxa_to_used_taxa_gbif}
used_taxa_final_match_gbif <-
  used_taxa_after_correction %>%
  filter(corrected == FALSE) %>%
  bind_rows(used_corrected_taxa_gbif) %>%
  arrange(id)
```

# Save used taxa with GBIF Backbone information

We save the used taxa with added taxonomic information from GBIF Backbone:

```{r match_used_corrected_taxa-save_taxa_after_final_match_gbif}
write_tsv(used_taxa_final_match_gbif, 
          path = here("data", "interim", "used_taxa_final_match_gbif.tsv"),
          na = "")
```
