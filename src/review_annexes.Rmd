...
**Important Note:**
_BIM species list_ in this context refers to the annex species list as available in the BIM database.
_Annex species list_ in this context refers to the official documents published by the respective instances (Habitat richtlijn, etc.)

The approach is to screen each annex list _separately_ for lacking and/or erroneous taxa. 

1. Retrieve and copypaste each annex species list in a [gsheets file](https://docs.google.com/spreadsheets/d/1Od5YYgMsTHvIFkrdeyWxkiARCya2yHKkAJrNM0-dnnw/edit#gid=1779940747). Each annex list is put into a different tab.
2. Import the annex species list in Rstudio.
3. To match the species list from the BIM database with the annex list, we need the _GBIF backbone_. In this way, we avoid nonmatches due to erroneous spelling or lacking authorship.
- Match bim species list with GBIF backbone using [taxa_gbif.tsv](https://github.com/inbo/ibge-bim-species/blob/master/data/input/taxa.tsv).
- Match annex species list with GBIF backbone by using `gbif_species_name_match()` function.
4. Screen nonmatching scientific names in annex species lists for errors and correct. 
5. TO BE CHANGED: Remove other non-matching scientificnames


# Pre-processing

Filter out minimal information from `bim_annex_species`: `acceptedname`, `taxonomicstatusen` and `annexcode`:

```{r}
bim_annex_species %<>% select(acceptedname, taxonomicstatusen, annexcode) 
```

Import`taxa_gbif.tsv` to add information from the GBIF backbone to the annex species list:

```{r}
taxa_gbif <- read.delim("~/GitHub/ibge-bim-species/data/interim/taxa_gbif.tsv")
```

Join GBIF information with annex_species:

```{r}
bim_annex_species <-
  bim_annex_species %>% 
    left_join(
      select(taxa_gbif, acceptedname, gbif_scientificName, gbif_kingdom, gbif_phylum, gbif_class, gbif_order, gbif_family, gbif_genus, gbif_matchType, gbif_canonicalName, gbif_status),
      by = "acceptedname") %>% 
  rename("bim_scientificName" = "acceptedname") %>% 
  distinct()
```
...
bim_annex_species %>% 
  ...
  filter(gbif_matchType  == "NONE" | is.na(gbif_matchType)) %>% 
  count()
```

**For now, we remove these taxa.** TO BE CHANGED (potentially)
...
Import annex species list. At this point they are saved as .csv files to allow import in Rstudio. 

```{r}
annex_bxl_2.1 <- read_delim(here("data", "input", "annex_bxl_2.1.csv"), delim = ",")
annex_bxl_2.2 <- read_delim(here("data", "input", "annex_bxl_2.2.csv"), delim = ",")
annex_bxl_2.3 <- read_delim(here("data", "input", "annex_bxl_2.3.csv"), delim = ",")
annex_bxl_2.4 <- read_delim(here("data", "input", "annex_bxl_2.4.csv"), delim = ",")
annex_bxl_2.5 <- read_delim(here("data", "input", "annex_bxl_2.5.csv"), delim = ",")
annex_bxl_3   <- read_delim(here("data", "input", "annex_bxl_3.csv"), delim = ",")
annex_bxl_4   <- read_delim(here("data", "input", "annex_bxl_4.csv"), delim = ",")
annex_bern_1   <- read_delim(here("data", "input", "annex_bern_1.csv"), delim = ",")
annex_bern_2   <- read_delim(here("data", "input", "annex_bern_2.csv"), delim = ",")
annex_bern_3   <- read_delim(here("data", "input", "annex_bern_3.csv"), delim = ",")
annex_bonn_1   <- read_delim(here("data", "input", "annex_bonn_1.csv"), delim = ",")
annex_bonn_2   <- read_delim(here("data", "input", "annex_bonn_2.csv"), delim = ",")
annex_hab_2   <- read_delim(here("data", "input", "annex_hab_2.csv"), delim = ",")
annex_hab_4   <- read_delim(here("data", "input", "annex_hab_4.csv"), delim = ",")
```
...
Match `annex_bxl_2.1` to the GBIF backbone:

```{r}
annex_bxl_2.1 %<>% gbif_species_name_match(
...
All scientific names match with the GBIF backbone.
...
non_matches_bxl_2.1 <- 
  bind_cols(
    as.tibble("BXL_2.1"),
    annex_bxl_2.1 %>% filter(matchType == "NONE") %>% count())
```
...
```{r}
unverifiable_records_bxl_2.1 <- tibble("BIM_2.1" = 0)
```
...
Inspect of `annex_bxl_2.1` and `bim_bxl_2.1`:

```{r}
summary_bxl_2.1 <-
  bind_cols(
    tibble("source" = rep("BIM_2.1",5)),
    bind_rows(
      annex_bxl_2.1 %>% 
...
```{r}
annex_bxl_2.2 %>% filter(matchType == "NONE") %>% arrange(scientific_name)
```
...
unverifiable_records_bxl_2.2 <- 
  unverifiable_records_bxl_2.2_taxa %>% 
    summarize(BIM_2.2 = sum(number_of_taxa))
unverifiable_records_bxl_2.2
```
...
```{r}
annex_bxl_2.3 %<>% filter(matchType != "NONE")
```
...
bind_rows(
  ...
  unverified_records_bern_2_taxa %>% mutate(source = "BERN_2"))
```
...
To do:
1. Deal with unmatched taxa
2. Deal with synonymy




## Inspect Annex BONN-2 Information

```{r}
bonn_2 %<>% gbif_species_name_match(
  name = "scientific_name",
  gbif_terms = gbif_terms,
  strict = TRUE)
```
...
bind_rows(
  bonn_2 %>% 
    summarize(number_of_taxa = n()) %>% 
    mutate(dataset = "Official Annex") %>% 
    select(dataset, number_of_taxa)
  ,
  bim_bonn_2 %>% 
    summarize(number_of_taxa = n()) %>% 
    mutate(dataset = "BIM database") %>% 
    select(dataset, number_of_taxa)
  ,
  bonn_2 %>% 
    inner_join(bim_bonn_2, by = "gbif_canonicalName") %>% 
    summarize(number_of_taxa = n()) %>% 
    mutate(dataset = "Full match") %>% 
    select(dataset, number_of_taxa)
  ,
  bim_bonn_2 %>% 
    anti_join(bonn_2, by = c("gbif_canonicalName")) %>%   
    summarize(number_of_taxa = n()) %>% 
    mutate(dataset = "BIM not in annex") %>% 
    select(dataset, number_of_taxa)
  ,
  bonn_2 %>% 
    anti_join(bim_bonn_2, by = c("gbif_canonicalName")) %>%   
    summarize(number_of_taxa = n()) %>% 
    mutate(dataset = "Annex not in BIM") %>% 
    select(dataset, number_of_taxa)
  )
```

The following taxa occur in `bim_bonn_2` but not in `bonn_2`:
...
unclear_records %<>% 
  summarize(records = "sum", number_of_taxa = sum(number_of_taxa)) %>% 
  bind_rows(unclear_records,.)

bind_rows(unclear_records,
          bim_bonn_2 %>% 
            anti_join(bonn_2, by = c("gbif_canonicalName")) %>%   
            summarize(number_of_taxa = n()) %>% 
            mutate(records = "BIM not in annex") %>% 
            select(records, number_of_taxa))
```

We filter out these records and inspect which are left over:

```{r}
bim_bonn_2 %>% anti_join(bonn_2, by = c("gbif_canonicalName")) %>% 
    arrange(bim_scientificName) %>% 
    filter(!(
      gbif_family == "Acanthizidae" |
      gbif_family == "Accipitridae" |
      gbif_family == "Acrocephalidae" |
      gbif_family == "Aegithalidae" |
      gbif_family == "Anatidae" |
      gbif_family == "Anseranatidae" |
      gbif_genus == "Anthropoides" |
      gbif_genus == "Antigone" |
      gbif_family == "Artamidae" |
      gbif_family == "Bernieridae" |
      gbif_family == "Cathartidae" |
      gbif_family == "Chaetopidae" |
      gbif_family == "Charadriidae" |
      gbif_family == "Cheloniidae" |
      gbif_family == "Cinclosomatidae" |
      gbif_family == "Cisticolidae" |
      gbif_family == "Dasyornithidae" |
      gbif_family == "Dermochelyidae" |
      gbif_family == "Eulacestomidae" |
      gbif_family == "Eupetidae" |
      gbif_family == "Falconidae" |
      gbif_family == "Falcunculidae" |
      gbif_genus == "Grus" |
      gbif_family == "Haematopodidae" |
      gbif_family == "Hyliotidae" |
      gbif_family == "Hylocitreidae" |
      gbif_family == "Ibidorhynchidae" |
      gbif_family == "Ifritidae" |
      gbif_family == "Leiotrichidae" |
      gbif_family == "Locustellidae" |
      gbif_family == "Machaerirhynchidae" |
      gbif_family == "Macrosphenidae" |
      gbif_family == "Maluridae" |
      gbif_family == "Melampittidae" |
      gbif_family == "Meliphagidae" |
      gbif_family == "Mohouidae" |
      gbif_family == "Monarchidae" |
      gbif_family == "Motacillidae" |
      gbif_family == "Muscicapidae" |
      gbif_family == "Oreoicidae" |
      gbif_family == "Oriolidae" |
      gbif_family == "Orthonychidae" |
      gbif_family == "Pachycephalidae" |
      gbif_family == "Panuridae" |
      gbif_family == "Pellorneidae" |
      gbif_family == "Petroicidae" |
      gbif_family == "Phoenicopteridae" |
      gbif_family == "Phylloscopidae" |
      gbif_family == "Picathartidae" |
      gbif_family == "Platysteiridae" |
      gbif_family == "Pluvianellidae" |
      gbif_family == "Pnoepygidae" |
      gbif_family == "Polioptilidae" |
      gbif_family == "Pomatostomidae" |
      gbif_family == "Psophodidae" |
      gbif_family == "Recurvirostridae" |
      gbif_family == "Regulidae" |
      gbif_family == "Rhagologidae" |
      gbif_family == "Rhinolophidae" |
      gbif_family == "Rhipiduridae" |
      gbif_family == "Scolopacidae" |
      gbif_family == "Scotocercidae" |
      gbif_family == "Stenostiridae" |
      gbif_family == "Sylviidae" |
      gbif_family == "Timaliidae" |
      gbif_family == "Turdidae" |
      gbif_family == "Vangidae" |
      gbif_family == "Vespertilionidae" |
      gbif_family == "Vireonidae" |
      gbif_family == "Zosteropida")
) %>% 
  select(gbif_scientificName, gbif_kingdom, gbif_phylum, gbif_class, gbif_family)
```

## Inspect Annex HAB-2 Information

```{r}
hab_2 %<>% gbif_species_name_match(
  name = "scientific_name",
  gbif_terms = gbif_terms,
  strict = TRUE)
```
...
Summarize inspection of `hab_2`, `bim_hab_2`:
...
bind_rows(
  ...
  ,
  hab_2 %>% 
    inner_join(bim_hab_2, by = "gbif_canonicalName") %>% 
    summarize(number_of_taxa = n()) %>% 
    mutate(dataset = "Full match") %>% 
    select(dataset, number_of_taxa)
  ,
  bim_hab_2 %>% 
    anti_join(hab_2, by = c("gbif_canonicalName")) %>%   
    summarize(number_of_taxa = n()) %>% 
    mutate(dataset = "BIM not in annex") %>% 
    select(dataset, number_of_taxa)
  ,
  hab_2 %>% 
    anti_join(bim_hab_2, by = c("gbif_canonicalName")) %>%   
    summarize(number_of_taxa = n()) %>% 
    mutate(dataset = "Annex not in BIM") %>% 
    select(dataset, number_of_taxa)
  )
...
The following taxa occur in `bim_hab_2` but not in `hab_2`:
