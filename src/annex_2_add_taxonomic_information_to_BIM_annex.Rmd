...
```{r}
unmatchted_bim_annex_taxa <- 
  bim_annex_species %>% 
    filter(gbif_matchType == "NONE") %>% 
    select(annexcode, acceptedname)
```

Export as `unmatchted_bim_annex_taxa.csv`

```{r}
write_csv(unmatchted_bim_annex_taxa, here("data", "processed", "unmatchted_bim_annex_taxa.csv"), na = "")
```
