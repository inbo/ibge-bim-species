...
We try to match by using `acceptedname` + `scientificnameauthorship`. We also provide kingdom and rank information to the match function. First, we prepare `taxa` for matching. In this step:
...
The taxa with `matchType` not equal to `EXACT` or `EXACT_WITHOUT_AUTHOR` are matched again. As third and final matching attempt, we try matching taxa without `scientificnameauthorship` and without passing kingdom as parameter. Please note that rank is always given to the match function.

```{r match_without_kingdom_acceptedname_only}
taxa_without_author_and_kingdom <-
  taxa_gbif %>%
  filter(!matchType %in% c("EXACT_WITHOUT_AUTHOR", 
                           "EXACT")) %>%
  select(names(taxa)) %>%
  gbif_species_name_match(name = "acceptedname",
                          rank = "rank_for_gbif",
...
As some of the matches might be wrong, we update `matchType` to `EXACT_WITHOUT_AUTHOR_AND_KINGDOM`, `FUZZY_WITHOUT_AUTHOR_AND_KINGDOM`:

```{r flag_matchType_without_author_and_kingdom}
taxa_without_author_and_kingdom <-
  taxa_without_author_and_kingdom %>%
  mutate(matchType = case_when(
    matchType == "EXACT" ~ "EXACT_WITHOUT_AUTHOR_AND_KINGDOM",
    matchType == "FUZZY" ~ "FUZZY_WITHOUT_AUTHOR_AND_KINGDOM",
    TRUE ~ matchType)
)
```
