...
We read the taxa which we will further use to build the improved `taxa` table. We select only the used taxa:

```{r apply_corrections_names_by_ref_file-read_used_taxa}
used_taxa <- read_tsv(here("data", "interim", "flagged_taxa.tsv"),
                      col_types = cols(
...
```{r apply_corrections_names_by_ref_file-n_used_taxa_to_correct}
corrected_taxa %>%
...
```{r apply_corrections_names_by_ref_file-correct_used_taxa}
...
```{r apply_corrections_names_by_ref_file-save_used_taxa_after_correction}
write_tsv(used_taxa_after_correction, 
